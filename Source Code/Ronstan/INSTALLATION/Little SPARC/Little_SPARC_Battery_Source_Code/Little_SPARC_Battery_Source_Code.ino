...
const int wakeUpTime = 5000000; // 5 seconds

unsigned long previousPrintTimeMillis = 0; // Variable to store the last time the voltage was printed
const unsigned long printInterval = 1000;  // Interval to print the voltage (1 second)
...
void mainFunction() {
  ...
  // Print voltage once every second
  if (currentMillis - previousPrintTimeMillis >= printInterval) {
    Serial.print("Voltage: ");
    Serial.println(voltageMillivolts);
    previousPrintTimeMillis = currentMillis;
  }

  // Check if it's time to trigger the heartbeat
  ...
  if (arcInitiated && (currentMillis - arcStartTime >= 6000)) {
    Serial.println("Warning: Arc has been initiated for 6 seconds");
    kirimArcCheck();
    arcStartTime = currentMillis; // Reset the start time to trigger the next warning after another 6 seconds
  }
}
...
void setup() {
  ...
  if (voltageMillivolts < 150) {
    Serial.println("Voltage below threshold, entering deep sleep...");

    // Configure the timer wake-up source
    esp_sleep_enable_timer_wakeup(wakeUpTime); // Wake up after 5 seconds
