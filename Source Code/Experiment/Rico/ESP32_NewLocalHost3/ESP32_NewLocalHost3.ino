...
const int MAX_AVERAGE_COUNT = 10;  // Adjust the maximum number of averages as needed
float averageArray[MAX_AVERAGE_COUNT];
int averageCount = 0;
...
void kirimArcOff() {
  // Include the array of averageCurrentDC values in the request
  String averageArrayString = "";
  for (int i = 0; i < averageCount; i++) {
    averageArrayString += String(averageArray[i], 3) + ",";
  }
  averageArrayString.remove(averageArrayString.length() - 1);  // Remove the trailing comma

  Status = "ArcOff";
  Link = "https://" + String(host) + "/kirimWaktu2.php?Timing=" + jmdS + "&Status=" + Status + "&MachineID=" + machineID + "&Area=" + area + 
         "&UID=" + uid + "&Status=" + Status + "&averageArray=" + averageArrayString + "&weldID=" + weldID;
  ...
  // Reset the average array and count for the next arc-on period
...
void syaratTiming() {
  ...
  if (currentRMS > 1) {
    ...
    // Check if 3 seconds have passed to calculate the average
    ...
    if (elapsedTime >= 3000) {
      ...
      if (counterCurrentDC > 0) {
        ...
        Serial.print("Average CurrentDC (3 seconds): ");
        Serial.println(averageCurrentDC);

        // Store the average current in the array
        if (averageCount < MAX_AVERAGE_COUNT) {
          averageArray[averageCount++] = averageCurrentDC;
        }

        // Reset the sumCurrentDC and counterCurrentDC for the next 3 seconds
        sumCurrentDC = 0;
        counterCurrentDC = 0;

        // Reset arcInitiatedTime for the next 3 seconds
        arcInitiatedTime = millis();
      }
    }
  } else {
    if (arcInitiated) {
      ...
      if (elapsedTime > 1000) { // Check if 5 seconds have passed
        ...
        // Here you can send the array when the arc stops
        kirimArcOff();
      }
    }
  }
