...
void loop() {
  // Check if it is time to read the voltage
  unsigned long currentMillis = millis();
  if (currentMillis - previousVoltageReadMillis >= voltageReadInterval) {
    previousVoltageReadMillis = currentMillis;

    // Read the voltage
    voltage = analogRead(32) * (3.3 / 4095.0); // Reading and converting voltage

    // Print the voltage to the serial monitor for debugging purposes
    Serial.print("Voltage: ");
    Serial.println(voltage);

    // If the voltage is below 1V and the functions haven't been executed yet
    if (voltage < 1.0 && !functionsExecuted) {
      sendLogout();
      delay(1000);
      kirimDone();
      functionsExecuted = true; // Mark that the functions have been executed
      voltageBelowThreshold = true; // Mark that the voltage was below threshold
    }

    // If the voltage was below threshold and now it is above 1V, reboot the ESP32
    if (voltageBelowThreshold && voltage >= 1.0) {
      Serial.println("Voltage is back. Rebooting ESP32...");
      ESP.restart();
    }
  }
