...
void kirimArcOff() {
  ...
  Link = "https://" + String(host) + "/kirimWaktu2.php?Timing=" + jmdS + "&Status=" + Status + "&MachineID=" + machineID + "&Area=" + area + 
         "&UID=" + uid + "&Status=" + Status + "&currentDC=" + averageCurrentDC + "&weldID=" + weldID + "&voltageDC=" + voltageString;
...
void loginMode() {
  ...
  while (currentState == LOGIN) {
    syaratTiming();
    readVoltage();
    dtostrf(getVoltageAverage(), 6, 2, voltageString);
    display.clearDisplay();
    ...
    display.print(getVoltageAverage());
    display.display();
    syaratTiming();
