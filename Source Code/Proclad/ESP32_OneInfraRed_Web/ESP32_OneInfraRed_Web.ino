...
const int ledPinWiFi = 33;  // Changed LED for WiFi status
const int ledPinArc = 25;   // LED for Arc status
...
void connectToWiFi() {
  ...
  if (wifiMulti.run() == WL_CONNECTED) {
    ...
    digitalWrite(ledPinWiFi, HIGH); // Turn on the WiFi LED
  } else {
    Serial.println("WiFi failed to connect after several tries");
    digitalWrite(ledPinWiFi, LOW); // Turn off the WiFi LED
  }
}
...
void setup() {
  ...
  pinMode(sensorPin32, INPUT); // Changed to sensorPin32

  // Set LED pins as outputs
  pinMode(ledPinWiFi, OUTPUT); // Changed to ledPinWiFi
  pinMode(ledPinArc, OUTPUT);  // Unchanged

  // Verify that pins are set correctly
  Serial.print("ledPinWiFi: ");
  Serial.println(ledPinWiFi);
  Serial.print("ledPinArc: ");
  Serial.println(ledPinArc);
...
void loop() {
  if (digitalRead(sensorPin32) == LOW) { // Changed to sensorPin32
    // If kirimArcOn hasn't been executed yet
    if (!arcOnExecuted) {
      // Print "Arc On" and execute kirimArcOn
      Serial.println("Arc On");
      digitalWrite(ledPinArc, HIGH); // Turn on the Arc LED
      kirimArcOn();
    ...
    if (arcOnExecuted && digitalRead(sensorPin32) == HIGH) { // Changed to sensorPin32
      ...
      digitalWrite(ledPinArc, LOW); // Turn off the Arc LED
      kirimArcOff();
